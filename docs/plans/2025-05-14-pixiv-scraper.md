...
**目标：** 实现一个单文件 Python 脚本，从 Pixiv 爬取符合指定条件的图片，按标签目录存储。

**架构：** 单脚本 + YAML 配置文件。使用 requests 做 HTTP 请求，串行下载，YAML 配置所有搜索条件。

**技术栈：** Python 3.9+, requests, PyYAML
...
```bash
ls -la
# 期望: config.yaml, images/, downloaded_ids.json
```
...
```bash
git init
git add config.yaml downloaded_ids.json
mkdir -p images && git add images/.gitkeep
...
```bash
python -m py_compile pixiv_scraper.py && echo "语法检查通过"
# 期望: 语法检查通过
```
...
```bash
git add pixiv_scraper.py
git commit -m "feat: 添加配置加载模块"
```

---

## Task 3: 实现 Pixiv API 客户端

**目标：** 创建 Pixiv API 封装，处理认证和请求

**文件：**
- 修改: `pixiv_scraper.py`（添加 120 行）
...
```bash
git add pixiv_scraper.py
git commit -m "feat: 添加 Pixiv API 客户端"
```
...
def determine_save_path(
    ...
    # 根据策略决定目录
    if matched_tags:
        if multi_tag_strategy == "first_match":
...
```bash
git add pixiv_scraper.py
git commit -m "feat: 添加主爬虫逻辑"
```

---

## Task 6: 创建 requirements.txt 和使用说明
...
4. 刷新页面，找到任意请求
5. 在请求头中复制 `cookie` 字段
6. 提取其中的 `PHPSESSID` 或 `session_id` 值
...
git commit -m "docs: 添加 requirements.txt 和 README"
```
...
```bash
python -c "import yaml; yaml.safe_load(open('config.yaml'))" && echo "✓ 配置文件格式正确"
```
...
- `config.yaml` - 配置文件
- `pixiv_scraper.py` - 主脚本
- `downloaded_ids.json` - 下载记录
- `images/` - 图片目录
- `requirements.txt` - 依赖
- `README.md` - 使用说明

**下一步：** 获取 Pixiv cookie，填入配置，运行脚本！
